import React from 'react';
import { parseStyle } from '@/utils/style';
import { cn } from '@/lib/utils';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';

const sizeToSpan = (size: string): { w: number; h: number } => {
  switch (size) {
    case 'rect_h': return { w: 2, h: 1 };
    case 'rect_v': return { w: 1, h: 2 };
    case 'large': return { w: 2, h: 2 };
    default: return { w: 1, h: 1 };
  }
};

export const DashboardWidget: React.FC<any> = ({ id, ExplicitList, size = 'square', title, style, edit_mode = false }) => {
  const span = sizeToSpan(size);

  const handleDragStart = (event: React.DragEvent<HTMLDivElement>) => {
    if (!edit_mode) return;
    event.stopPropagation();
    event.dataTransfer.setData('application/x-dashboard-widget', String(id || ''));
    event.dataTransfer.setData('application/x-dashboard-widget-w', String(span.w));
    event.dataTransfer.setData('application/x-dashboard-widget-h', String(span.h));
    event.dataTransfer.effectAllowed = 'move';
  };

  return (
    <Card
      draggable={Boolean(edit_mode)}
      onDragStart={handleDragStart}
      className={cn('flex h-full min-h-0 flex-col overflow-hidden', edit_mode && 'cursor-move')}
      style={parseStyle(style)}
      data-widget-id={id}
      data-grid-w={span.w}
      data-grid-h={span.h}
    >
      {title ? (
        <CardHeader className="flex flex-row items-center justify-between gap-2 border-b py-2">
          <CardTitle className="truncate text-xs font-semibold uppercase tracking-wide">{title}</CardTitle>
          {edit_mode ? <span className="text-[10px] text-muted-foreground">drag</span> : null}
        </CardHeader>
      ) : null}
      <CardContent className="min-h-0 flex-1 overflow-auto p-2.5">{ExplicitList}</CardContent>
    </Card>
  );
};
