...
export const EditableList: React.FC<any> = ({
  ...
  const fieldLabel = getLiteral(item_label);
  ...
  return (
    <div className="grid gap-3" style={parseStyle(style)}>
      {fieldLabel ? <Label>{fieldLabel}</Label> : null}
      <div className="grid gap-2">
        {items.map((item, index) => (
          <div key={`${id}_${index}`} className="flex min-w-0 items-center gap-2">
            {schema.type === 'select' ? (
              <select
                value={item}
      ...
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="outline" size="sm" onClick={() => publishItems([...items, newItemValue()])}>
          {addText}
        </Button>
        <Button type="button" size="sm" onClick={submit} disabled={isLoading} className="gap-2">
          {isLoading ? <i className={cn('ri-loader-4-line animate-spin')} /> : null}
          {submitText}
        </Button>
      </div>
    </div>
