...
export const Form: React.FC<any> = ({
  ...
  const renderNode = (node: any, path: string): React.ReactNode => {
        ...
        return (
          <div className="flex flex-col gap-4 md:flex-row">
            {renderedChildren.map(({ child, index, node: renderedNode }) => {
              ...
              return (
                <div key={`${path}_row_${index}`} className="min-w-0" style={{ flex: `${stretch} 1 0%` }}>
                  {renderedNode}
                </div>
              );
            })}
    ...
    return <div>{renderField(node, key)}</div>;
  };
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="mt-4 flex justify-end">
        <Button type="submit" disabled={submitDisabled} className="flex items-center gap-2">
          {isSubmitLoading && <i className="ri-loader-4-line text-base animate-spin" />}
