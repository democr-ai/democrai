...
export const DatePicker: React.FC<any> = ({
  ...
  const validate = React.useCallback((next: string): string => {
    ...
    if (!parsed) return 'Invalid value for selected format.';
  ...
  return (
    <div className="grid gap-1.5" style={parseStyle(style)}>
      {label ? (
        <Label className={cn(shownError && 'text-destructive')}>
          {getLiteral(label)}
        </Label>
      ) : null}
      <Input
        ...
        className={cn(shownError && 'border-destructive/50 focus-visible:ring-destructive/50 bg-destructive/5')}
      />
      {shownError ? (
