    ...
    return (
      <>
        ...
        <UncontrolledTooltip placement="right" target={targetId}>
          {hasNotifications ? `${pendingCount} notifiche pendenti` : 'Notifiche'}
        </UncontrolledTooltip>
      </>
    );
  }
  ...
  return (
    ...
    >
      {visibleItems.map((entry: any, index: number) => {
        ...
        return (
          <React.Fragment key={btnId}>
            ...
            {label && (
              <UncontrolledTooltip placement="right" target={btnId}>
                {label}
              </UncontrolledTooltip>
            )}
          </React.Fragment>
      ...
      {tag === APP_MAIN_LIST_TAG && overflowItems.length > 0 ? (
        <SmartDropdown
          className="main-sidebar-dropdown"
          menuClassName="main-sidebar-dropdown-menu shadow"
          placement="right-start"
          fallbackPlacements={['left-start', 'top-start', 'bottom-start']}
          button={(
            <button type="button" className="main-sidebar-nav-btn" id="sidebar-more-btn">
              <i className="ri-more-fill main-sidebar-nav-icon" />
            </button>
          )}
        >
          <UncontrolledTooltip placement="right" target="sidebar-more-btn">Altro</UncontrolledTooltip>
            {overflowItems.map((entry: any, index: number) => {
              ...
              return (
                <button
                  ...
                  className={`dropdown-item d-flex align-items-center ${active ? 'active' : ''}`}
                  onClick={() => invokeEntry(entry)}
                >
                  <SidebarIcon icon={entry?.icon} className="me-2" />
                  {label}
                </button>
              );
            })}
      ...
      {showLanguageMenu && (
        <>
          ...
          <UncontrolledTooltip placement="right" target="theme-toggle-btn">
            {clientTheme === 'dark' ? 'Tema Scuro' : 'Tema Chiaro'}
          </UncontrolledTooltip>
        </>
      )}
      ...
      {showLanguageMenu && (
        <SmartDropdown
          ...
          menuClassName="main-sidebar-dropdown-menu shadow"
          placement="right-end"
          fallbackPlacements={['right-start', 'left-end', 'top-start', 'bottom-start']}
          button={(
            <button type="button" className="main-sidebar-nav-btn fw-bold small" id="lang-toggle-btn">
              {currentLanguage.toUpperCase()}
            </button>
          )}
        >
          <UncontrolledTooltip placement="right" target="lang-toggle-btn">Lingua</UncontrolledTooltip>
            {languageOptions.map((entry: any) => (
              <button
                type="button"
                key={entry.value}
                className={`dropdown-item ${entry.value === currentLanguage ? 'active' : ''}`}
