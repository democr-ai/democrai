...
export const GridDropZone: React.FC<any> = ({
  ...
  const renderCellOverlay = () => {
    ...
    for (let row = 0; row < rowCount; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        cells.push(
          <div key={`cell_${row}_${col}`} className={cn("a2ui-grid-cell position-relative border", isHover && "a2ui-grid-cell-hover", isOccupied && "a2ui-grid-cell-occupied")}>
            {edit_mode && !isOccupied && (
              <div className="d-flex h-100 w-100 align-items-center justify-content-center">
                <Button color="link" size="xs" onClick={() => setAddMenuCell(isMenuOpen ? null : { row, col })} className="a2ui-grid-add-button text-decoration-none">
                  <i className="ri-add-line fs-5" />
                </Button>
              </div>
            )}
            {edit_mode && !isOccupied && isMenuOpen && (
              <div className="a2ui-grid-add-menu position-absolute top-100 start-50 translate-middle-x z-index-20 border p-1 mt-2" style={{ minWidth: '140px', zIndex: 1000 }}>
                {addOptions.map(opt => (
                  <button key={opt.size} type="button" className="a2ui-grid-add-menu-item btn btn-xs btn-link text-start w-100 small text-decoration-none px-2 py-1" onClick={() => {
  ...
  return (
    <div id={id} ref={gridRef} className="a2ui-grid-dropzone position-relative w-100" style={{ ...parseStyle(style), minHeight: gridHeight }} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={() => edit_mode && setHoverCell(null)} onClick={ev => !ev.target?.closest('button') && setAddMenuCell(null)}>
      <div className="a2ui-grid-surface d-grid w-100 border p-2" style={{ gridTemplateColumns: `repeat(${cols}, ${cellSize}px)`, gridTemplateRows: `repeat(${rowCount}, ${cellSize}px)`, gap: cellGap, height: gridHeight }}>
        {edit_mode && renderCellOverlay()}
      </div>
      <div className="position-absolute d-grid p-2" style={{ inset: 0, pointerEvents: 'none', gridTemplateColumns: `repeat(${cols}, ${cellSize}px)`, gridTemplateRows: `repeat(${rowCount}, ${cellSize}px)`, gap: cellGap, height: gridHeight }}>
        {widgets.map(meta => {
          const child = childById.get(meta.id);
          if (!child) return null;
          return (
            <div key={meta.id} className="a2ui-grid-widget-slot overflow-hidden" style={{ ...placeStyleFor(meta), pointerEvents: 'auto' }}>
