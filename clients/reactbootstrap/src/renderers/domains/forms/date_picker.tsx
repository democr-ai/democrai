...
export const DatePicker: React.FC<any> = ({
  ...
  return (
    <FormGroup className="a2ui-field a2ui-date-picker" style={parseStyle(style)}>
      {labelText && (
        <Label for={id} className={shownError ? 'text-danger' : ''}>
          {labelText}
        </Label>
      )}
      <div className="a2ui-date-picker-control">
        <span className="a2ui-date-picker-icon" aria-hidden="true">
          <i className={hasTime ? 'ri-time-line' : 'ri-calendar-line'} />
        </span>
        <Input
          id={id}
          value={localValue}
          ...
          valid={shownError ? false : undefined}
        />
      </div>
      {shownError && <FormFeedback>{shownError}</FormFeedback>}
    </FormGroup>
  );
};
