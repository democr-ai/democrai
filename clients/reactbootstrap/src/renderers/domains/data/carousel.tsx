...
export const Carousel: React.FC<any> = ({
  ...
  if (!itemTemplate) {
    return (
      <Card style={parseStyle(style)} className="border">
        <CardBody className="p-4 text-center text-muted small">
          Carosello: nessun template configurato.
        </CardBody>
      </Card>
    );
  }
  ...
  return (
    <Card style={parseStyle(style)} className="a2ui-carousel border bg-light">
      <CardBody className="p-3 d-flex flex-column gap-3">
        <div className="a2ui-carousel-viewport overflow-hidden rounded bg-white" ref={emblaRef}>
          <div className="d-flex">
            {items.map((item: any, index: number) => (
              <div key={item?.id || `${id}_slide_${index}`} className="flex-shrink-0 flex-grow-0" style={{ flexBasis: '100%', minWidth: 0 }}>
                <div 
        ...
        <div className="a2ui-carousel-nav d-flex align-items-center justify-content-center gap-2">
          {showArrows && (
            <Button color="primary" outline size="sm" className="a2ui-carousel-arrow rounded-circle p-1" onClick={() => emblaApi?.scrollPrev()}>
              <i className="ri-arrow-left-s-line fs-5" />
            </Button>
          )}
          
          <div className="a2ui-carousel-dots d-flex align-items-center gap-1 mx-2">
            {showDots && items.map((_: any, idx: number) => (
              <button
                key={`dot_${idx}`}
                type="button"
                className={cn(
                  "a2ui-carousel-dot border-0 p-0 transition-all",
          ...
          {showArrows && (
            <Button color="primary" outline size="sm" className="a2ui-carousel-arrow rounded-circle p-1" onClick={() => emblaApi?.scrollNext()}>
