...
export const SequenceDiagram: React.FC<any> = ({ title, participants = [], messages = [], mermaid, style, height = 360 }) => {
  ...
  return (
    <Card style={parseStyle(style)} className="a2ui-sequence-diagram border shadow-sm overflow-hidden">
      {title && (
        <CardHeader className="a2ui-sequence-diagram-header bg-light border-bottom">
          <CardTitle className="m-0 xsmall text-uppercase fw-bold text-muted">{getLiteral(title)}</CardTitle>
        </CardHeader>
      )}
      <CardBody className="p-0">
        {!model.participants.length ? (
          <div className="p-4 text-center text-muted small">Nessun dato del diagramma di sequenza disponibile.</div>
        ) : (
          <div className="overflow-auto bg-white" style={{ maxHeight: Math.max(220, Number(height) || 360) }}>
            <svg width={layout.width} height={layout.contentHeight} role="img" aria-label="diagramma di sequenza">
              {model.participants.map(p => {
                const centerX = layout.laneX.get(p.id) || 0;
                return (
                  <g key={p.id}>
                    <rect x={centerX - layout.headerWidth / 2} y={layout.topPad} width={layout.headerWidth} height={layout.headerHeight} rx={4} fill="#f8f9fa" stroke="#dee2e6" strokeWidth={1.5} />
                    <text x={centerX} y={layout.topPad + 22} textAnchor="middle" fill="#212529" fontSize={11} fontWeight={700} style={{ fontFamily: 'monospace' }}>{p.label}</text>
                    <line x1={centerX} y1={layout.lineTop} x2={centerX} y2={layout.contentHeight - 20} stroke="#adb5bd" strokeDasharray="4 4" strokeWidth={1} />
                  </g>
                );
              })}
              {model.messages.map((m, idx) => {
                ...
                const dashed = m.type === 'reply', stroke = "#495057";
                if (sx === tx) {
                  return (
                    <g key={idx}>
                      <path d={`M ${sx} ${y} L ${sx + SELF_LOOP_WIDTH} ${y} L ${sx + SELF_LOOP_WIDTH} ${y + SELF_LOOP_HEIGHT} L ${sx} ${y + SELF_LOOP_HEIGHT}`} fill="none" stroke={stroke} strokeWidth={1.5} strokeDasharray={dashed ? '4 3' : undefined} />
                      <polygon points={arrowHead(sx, y + SELF_LOOP_HEIGHT, false)} fill={stroke} />
                      {m.text && <text x={sx + SELF_LOOP_WIDTH + 8} y={y + 14} fill="#6c757d" fontSize={10} style={{ fontFamily: 'monospace' }}>{m.text}</text>}
                    </g>
                  );
                }
                return (
                  <g key={idx}>
                    <line x1={sx} y1={y} x2={tx} y2={y} stroke={stroke} strokeWidth={1.5} strokeDasharray={dashed ? '4 3' : undefined} />
                    <polygon points={arrowHead(tx, y, tx > sx)} fill={stroke} />
                    {m.text && <text x={Math.min(sx, tx) + Math.abs(tx - sx) / 2} y={y - 8} textAnchor="middle" fill="#6c757d" fontSize={10} style={{ fontFamily: 'monospace' }}>{m.text}</text>}
                  </g>
                );
              })}
            </svg>
