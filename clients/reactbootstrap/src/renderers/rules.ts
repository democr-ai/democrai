import { readClientStateValue, type ClientStateScope, type ClientStateSnapshot } from '@/state/clientState';

export interface RuleOptions {
  surfaceModel?: any;
  stateModel?: ClientStateSnapshot;
...
const normalizeScope = (scope: string): ClientStateScope => {
  const scopeKey = String(scope || '').trim().toLowerCase();
  if (scopeKey === 'page' || scopeKey === 'global') return scopeKey;
  return 'auto';
};

const getScopedStateValue = (stateModel: ClientStateSnapshot | undefined, scope: string, path: string): any => {
  const normalizedPath = String(path || '').trim();
  if (!normalizedPath) return undefined;
  return readClientStateValue(stateModel, normalizeScope(scope), normalizedPath);
};

const getSurfaceDataValue = (surfaceModel: any, path: string): any => {
  const normalizedPath = String(path || '').trim();
  if (!normalizedPath) return undefined;
  return getByPath(surfaceModel, normalizedPath.replace(/^\//, '').replace(/\//g, '.'));
};

export const resolveOperand = (operand: any, options: RuleOptions): any => {
  if (operand === undefined || operand === null) return operand;

  // Handle store bindings
  if (isObject(operand)) {
    const type = String(operand.type || '').trim().toLowerCase();
    
    // Explicit store binding: { type: 'store', path: '...', scope: '...' }
    if (type === 'store') {
    ...
    if (operand.path && Object.keys(operand).length <= 2) {
    ...
    // Literal binding: { type: 'literal', value: '...' }
    if (type === 'literal') {
  ...
  // Handle string bindings: @state/..., $state..., $item...
...
export const evaluateRule = (rule: any, options: RuleOptions, defaultVal = false): boolean => {
  ...
  if (typeof rule === 'string') {
    // Resolve string if it looks like a binding
  ...
  const results = conditions.map(c => evaluateCondition(c, options));
...
export const isSuperRole = (role: string): boolean => {
...
export const hasRequiredPermissions = (
  ...
  const required = Array.isArray(requiredPermissions)
    ? requiredPermissions.map((entry) => String(entry || '').trim()).filter(Boolean)
    : [];
...
export const isNestedActionVisible = (
  spec: any,
  options: RuleOptions,
  role: string,
  permissions: string[],
): boolean => {
  const showIf = spec?.show_if;
  if (showIf !== undefined && !evaluateRule(showIf, options, true)) {
    return false;
  }

  const hideIf = spec?.hide_if;
  if (hideIf !== undefined && evaluateRule(hideIf, options, false)) {
    return false;
  }

  return hasRequiredPermissions(spec?.required_permissions, role, permissions);
};
...
export const resolveBindings = (data: any, options: RuleOptions): any => {
  if (typeof data === 'string') {
    ...
    if (options.item) {
      ...
      const replacedMustache = data.replace(/\{\{\s*([^{}]+?)\s*\}\}/g, (match: string, rawKey: string) => {
        const itemVal = getByPath(options.item, rawKey);
        if (itemVal !== undefined) return String(itemVal);
        const stateVal = getScopedStateValue(options.stateModel, 'auto', String(rawKey || '').trim());
  ...
  if (isObject(data)) {
    ...
    if (bindingType === 'store') {
      ...
      if ((data as any).default !== undefined) return (data as any).default;
      return '';
    }
    ...
    // Keep implicit data-model bindings limited to binding-shaped objects.
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === 'children') {
        ...
        next[key] = resolveBindings(value, {
          ...options,
          allowImplicitPath: key === 'context' ? false : true,
        });
      }
    });
    return next;
