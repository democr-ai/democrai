...
export function useA2UI(options?: string | UseA2UIOptions) {
  ...
  const browserNavTimerRef = useRef<number | null>(null);
  const suppressBrowserRouteEventRef = useRef<string>('');
  ...
  const handleMessageData = useCallback((msg: any) => {
    ...
    if (msg.current_path !== undefined) {
      ...
      if (currentBrowserPath !== nextPath) {
        suppressBrowserRouteEventRef.current = nextPath;
  ...
  useEffect(() => {
    ...
    const armConnectTimer = () => {
      if (cancelled || connectTimer != null) return;
    ...
    const scheduleInitialConnect = () => {
      if (cancelled || connectTimer != null || firstPaintFrame != null || secondPaintFrame != null) return;
    ...
    return () => {
      ...
      if (browserNavTimerRef.current != null) window.clearTimeout(browserNavTimerRef.current);
      clearTokenRefreshTimer();
      runtimeRef.current?.dispose();
      const currentTransport = transportRef.current;
  ...
  useEffect(() => {
    const onPopState = () => {
      const nextPath = readBrowserRoute();
      if (suppressBrowserRouteEventRef.current === nextPath) {
        suppressBrowserRouteEventRef.current = '';
        browserPathRef.current = nextPath;
        return;
      }

      if (browserNavTimerRef.current != null) {
        window.clearTimeout(browserNavTimerRef.current);
      }
      browserNavTimerRef.current = window.setTimeout(() => {
        browserNavTimerRef.current = null;
        const settledPath = readBrowserRoute();
        if (settledPath === browserPathRef.current) return;
        browserPathRef.current = settledPath;
        hasSyncedBrowserPathRef.current = true;
        sendRaw({
          jwt: jwtRef.current || undefined,
          current_path: settledPath,
          userAction: {
            name: 'nav',
            context: { path: settledPath },
          },
        });
      }, 50);
    };
