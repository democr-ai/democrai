...
def _extractor_install_launch_state(
    extractor_id: str,
    *,
    env: dict[str, str],
) -> dict[str, Any]:
...
def _start_extractor_install_process(
    command: list[str],
    *,
    env: dict[str, str],
    extractor_id: str,
    ...
    return sandbox_launcher.popen(
        ...
        text=False,
        state=_extractor_install_launch_state(extractor_id, env=env),
    )
    ...
    *,
    ...
    try:
        process = await asyncio.to_thread(
            _start_extractor_install_process,
            command,
            env=env,
            extractor_id=extractor_id,
        )
