"""Classification providers used by knowledge projections."""
...
class ClassificationProvider(Protocol):
    """Protocol implemented by knowledge classification providers."""

    model_id: str
    model_version: str
    model_registry_id: int

    def classify(self, texts: Sequence[str]) -> list[object]: ...


_CLASSIFICATION_EXECUTOR = ThreadPoolExecutor(
    max_workers=2,
    thread_name_prefix="knowledge-classify",
)
...
def _run_sync(coro):
    ...
    return _CLASSIFICATION_EXECUTOR.submit(
        lambda queued_coro: ctx.run(asyncio.run, queued_coro),
        coro,
    ).result()


class ModelRegistryClassificationProvider:
    """Classification provider bound to one explicit model_registry row."""
    ...
    def classify(self, texts: Sequence[str]) -> list[object]:
        """Synchronously classify texts through the configured model."""
        if not texts:
            return []
        return _run_sync(self._classify_async(list(texts)))

    async def _classify_async(self, texts: list[str]) -> list[object]:
        ...
        if result.get("status") != "ok" or "provider" not in result:
            raise RuntimeError(
                "Failed to resolve classification provider for "
                f"model_registry_id={self.model_registry_id}: {result}"
            )
        return list(
            await result["provider"].classify(texts=texts, options=self.options) or []
        )
