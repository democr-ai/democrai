...
async def render(params: dict[str, Any], session: dict) -> sdk.ui.Builder:
    builder = sdk.ui.load("utils/ui/yaml/models/catalog_resource_warning")
    ...
    builder.set_data(
        "/system/model/catalog/resource_warning",
        {
            "title": f"Resource warning for {label}",
            "text": _warning_text(resource_warning),
        },
    )
    confirm = builder.get_component("system_model_catalog_resource_warning_confirm")
