...
from democrai.sdk.engines import RerankOptions
...
@action("test_engine_model_rerank")
@permission_required(["system.engine.model.manage"])
async def test_engine_model_rerank(ctx: dict[str, Any], module_sdk):
    method = "rerank"
    ...
    try:
        ...
        options = _rerank_options(test_ctx)
        await append_structured_trace(
            ...
            payload={
                ...
                "query_chars": len(test_ctx.prompt),
                "items": len(texts),
...
def _rerank_options(test_ctx) -> RerankOptions:
    return RerankOptions(
        top_k=_optional_int_value(_runtime_value(test_ctx, "top_k")),
        max_tokens_per_doc=_optional_int_value(
            _runtime_value(test_ctx, "max_tokens_per_doc")
        ),
    )
