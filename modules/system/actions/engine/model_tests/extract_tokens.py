...
@action("test_engine_model_extract_tokens")
@permission_required(["system.engine.model.manage"])
async def test_engine_model_extract_tokens(ctx: dict[str, Any], module_sdk):
    method = "extract_tokens"
    ...
    try:
        await append_structured_trace(
            ...
            stream_id,
            method=method,
            label="extract tokens request",
            payload={"model_row_id": test_ctx.row_id, "text_chars": len(test_ctx.prompt)},
        )
        ...
        response = await provider.extract_tokens(text=test_ctx.prompt)
        ...
        await append_structured_success(
            ...
            stream_id,
            method=method,
            result_label="token extraction results",
            result=result,
