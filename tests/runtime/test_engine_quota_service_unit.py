...
from democrai.core.application.ai.engine.quotas.types import METRIC_REQUESTS
...
class _UsageStore:
    ...
    def aggregate_ai_model_usage(self, **kwargs):
        self.calls.append(kwargs)
        return self.values.get(kwargs.get("metric_type"), self.value)
...
def test_engine_quota_all_limit_blocks(quota_env):
    ...
    assert decision.metric_type == METRIC_TOTAL_TOKENS
    assert decision.used_value == 10
    assert decision.limit_value == 10
...
def test_engine_quota_requests_metric_blocks_by_request_count(quota_env):
    _limit(
        quota_env,
        period_count=2,
        period_unit=PERIOD_MINUTE,
        scope_type=SCOPE_ALL,
        metric_type=METRIC_REQUESTS,
        limit=10,
    )
    quota_env.store.values[METRIC_REQUESTS] = 10
    ...
    assert decision.allowed is False
    assert decision.metric_type == METRIC_REQUESTS
    assert decision.used_value == 10
    assert decision.limit_value == 10
    assert quota_env.store.calls[-1]["metric_type"] == METRIC_REQUESTS


def test_engine_quota_multiple_metrics_on_same_counter_are_all_binding(quota_env):
    with quota_env.SessionLocal() as session:
        counter = EngineQuotaCounter(name="mixed", period_count=2, period_unit=PERIOD_MINUTE)
        session.add(counter)
        session.flush()
        session.add_all(
            [
                EngineQuotaLimit(
                    counter_id=counter.id,
                    engine_row_id=quota_env.engine_row_id,
                    scope_type=SCOPE_ALL,
                    scope_id=None,
                    metric_type=METRIC_TOTAL_TOKENS,
                    limit_value=100,
        ...
        session.commit()
    quota_env.store.values[METRIC_TOTAL_TOKENS] = 10
    quota_env.store.values[METRIC_REQUESTS] = 3
    ...
    assert decision.allowed is False
    assert decision.metric_type == METRIC_REQUESTS
    assert [call["metric_type"] for call in quota_env.store.calls] == [
        METRIC_TOTAL_TOKENS,
        METRIC_REQUESTS,
    ]
...
def test_engine_quota_windows_are_rolling(quota_env, monkeypatch):
    ...
    for unit, started_at in expected.items():
        ...
        _limit(quota_env, period_count=1, period_unit=unit, scope_type=SCOPE_ALL, limit=100)
        decision = check_engine_quota(
            engine_registry_id=quota_env.engine_row_id,
            user_id=quota_env.user_id,
            request_context={"user": quota_env.user_id},
...
def test_engine_quota_windows_use_period_count(quota_env, monkeypatch):
    now = datetime(2026, 3, 31, 12, 0, 0)
    monkeypatch.setattr(quota_service_mod, "utc_now_naive", lambda: now)
    expected = {
        PERIOD_MINUTE: now - timedelta(minutes=2),
        PERIOD_HOUR: now - timedelta(hours=2),
        PERIOD_DAY: now - timedelta(days=2),
        PERIOD_WEEK: now - timedelta(weeks=2),
        PERIOD_MONTH: datetime(2026, 1, 31, 12, 0, 0),
    }

    for unit, started_at in expected.items():
        quota_env.store.calls.clear()
        _limit(quota_env, period_count=2, period_unit=unit, scope_type=SCOPE_ALL, limit=100)
        decision = check_engine_quota(
            engine_registry_id=quota_env.engine_row_id,
            user_id=quota_env.user_id,
            request_context={"user": quota_env.user_id},
