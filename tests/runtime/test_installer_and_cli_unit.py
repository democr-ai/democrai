...
def test_installer_additional_branches(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("democrai.core.runtime.dependencies.engine_env.has_engine_env_context", lambda: True)
    monkeypatch.setattr(
        "democrai.core.runtime.dependencies.engine_env.get_engine_local_env_path",
        lambda engine_id=None, *, create=True: tmp_path / "engine_env",
    )
    ...
    monkeypatch.setattr(
        "democrai.core.runtime.dependencies.extractor_env.has_extractor_env_context",
        lambda: False,
    )
    monkeypatch.setattr(installer_mod, "get_target_dir", lambda: tmp_path)
    monkeypatch.setattr(installer_mod, "_ensure_uv_venv", lambda **_k: None)
...
def test_install_dependencies_batches_target_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "democrai.core.runtime.dependencies.engine_env.get_engine_venv_path",
        lambda engine_id=None, *, create=True: tmp_path / ".venv",
    )
    monkeypatch.setattr(
        "democrai.core.runtime.dependencies.engine_env.get_engine_venv_python_path",
        lambda engine_id=None, *, create=True: (
            tmp_path / ".venv" / ("Scripts" if os.name == "nt" else "bin") / ("python.exe" if os.name == "nt" else "python")
        ),
    )
    monkeypatch.setattr(
        "democrai.core.runtime.dependencies.engine_env.get_engine_venv_site_packages_path",
        lambda engine_id=None, *, create=True: installer_mod._venv_site_packages_path(tmp_path / ".venv"),
    )
    pip_calls = []
    monkeypatch.setattr(
        installer_mod,
