...
def _requires_real_sandbox() -> None:
    if os.environ.get(RUN_ENV) != "1":
        pytest.skip(f"set {RUN_ENV}=1 to run real Windows sandbox diagnostics")
    if sys.platform != "win32":
        pytest.skip("real Windows sandbox diagnostics are Windows-only")
    if not hasattr(ctypes, "windll") or not hasattr(ctypes.windll, "userenv"):
        raise AssertionError("windows_sandbox_appcontainer_unavailable")
    missing = [
        command
        for command in ("powershell.exe", "icacls.exe", "CheckNetIsolation.exe")
        if shutil.which(command) is None
    ]
    if missing:
        raise AssertionError(f"windows_sandbox_required_tool_missing:{','.join(missing)}")
...
def _python_env(runtime_root: Path) -> dict[str, str]:
    ...
    env["LOCALAPPDATA"] = str(runtime_root / "localappdata")
    env["APPDATA"] = str(runtime_root / "appdata")
...
def _short_runtime_root(name: str) -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"dc-windows-{os.getpid()}-{digest}"
...
def sandbox_harness(tmp_path: Path) -> Path:
    ...
    return _write(
        ...
        def set_probe_request_context():
            ...
            return set_req_ctx(
                RequestContext(
                    request_id="windows-sandbox-test",
                    user=1,
                    role="admin",
                    organization_id=1,
                    access_level=100,
                    channel="ipc",
                    app=app_ctx(),
                    session_key="windows-sandbox-session",
                    module_name="system",
                    stream_id="windows-sandbox-stream",
                )
            )
        ...
        def make_fake_extensions():
            root = track_temp_dir("democrai_windows_extensions_")
        ...
        class FakeMediaProvider:
            def __init__(self):
                self.root = track_temp_dir("democrai_windows_media_")
        ...
        def make_fake_skill():
            root = track_temp_dir("democrai_windows_skill_")
        ...
        def case_sandboxed_process_cannot_control_helper():
            attacker = track_temp_dir("democrai_windows_helper_attack_") / "helper_attack.py"
        ...
        def _run_skill_tool():
            ...
            async def _run():
                context = create_ai_pipeline_context(root_method="windows_sandbox_skill_tool", request_id="windows-sandbox-skill-tool")
        ...
        def case_mcp_http_registry_endpoint():
            ...
            try:
                ...
                thread.start()
                db_path = track_temp_dir("democrai_windows_mcp_db_") / "mcp.sqlite"
        ...
        def main():
            ...
            cases = {{
                ...
                "appcontainer_acl_filesystem_enforcement": case_appcontainer_acl_filesystem_enforcement,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "appcontainer_acl_filesystem_enforcement",
...
def test_real_sandbox_end_to_end_windows(case: str, sandbox_harness: Path):
